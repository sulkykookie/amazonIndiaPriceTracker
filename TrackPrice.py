...
title = soup.find(id = "productTitle").get_text()	#Fetches the name of the product by finding span with tage "productTitle"

filename = title.strip()+".csv"						#Creates new .csv file for the spcific product

price = soup.find(id = "priceblock_ourprice").get_text()	#Fetches the price of the product by finding span with tage "priceblock_ourprice"
price = price[2:]											#Removes the Rupee symbol
